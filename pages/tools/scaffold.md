...
| `NEXT_PUBLIC_PDS_URL` | PDS URL (e.g. `https://dev.certified.app` for staging, or `https://epds1.test.certified.app` for bleeding-edge testing — see [Certified services](/reference/certified-services)) |
| `NEXT_PUBLIC_EPDS_URL` | ePDS URL (e.g. `https://dev.certified.app`) (optional; required only for email/passwordless login) |
