...
A reference indexer that indexes hypercert records across the network and exposes them via a GraphQL API. Other indexers exist — see [Hyperindex](/services/hyperindex).
...
The server where your records are stored. You interact with it through the ATProto API — you don't need to manage it directly. You can use a [Certified-operated PDS](/services/certified-pdss), Bluesky's, or self-host one. Records are portable between PDS instances.
