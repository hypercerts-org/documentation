...
async function fetchGitHubResponse(url, token, accept, userAgent) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`${url} returned ${response.status} ${response.statusText || ''}. Check repo, ref, path, and DOCS_SOURCE_TOKEN. For local authenticated requests, run gh auth login. ${body}`.trim());
  }
  return response;
}
...
async function fetchLatestRelease(source, token) {
  ...
  if (!response.ok) throw new Error(`Release metadata for ${source.id} returned ${response.status}. Refusing to label a failed lookup as unreleased. Set DOCS_SOURCE_TOKEN or run gh auth login locally.`);
