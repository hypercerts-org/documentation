const RELEASE_MARKER = /^{% (release-latest|release-summary|release-cards|protocol-history) \/%}$/gm;
...
function renderReleaseFragment(kind, catalog) {
  ...
  if (kind === 'release-latest') {
    return `**[Hypercerts Protocol ${latest.version}](/changes): ${latest.title}.** ${latest.summary}`;
  }
  if (kind === 'release-summary') {
    return `## Hypercerts Protocol ${latest.version}\n\n**${latest.title}** · ${latest.date}\n\n${latest.summary}\n\n${latest.changes.map(change => `- ${change}`).join('\n')}\n\n${latest.compatibility}\n\n[Read the protocol changelog](/changes/protocol) · [Lexicons ${latest.sourceVersion} release](${sourceLink(latest)})${latest.blogUrl ? ` · [Read the release article](${latest.blogUrl})` : ''}`;
  }
  if (kind === 'release-cards') {
    const cards = [
      `{% card-link title="Protocol changelog" href="/changes/protocol" %}\nMajor and minor releases, from 1.0 to ${latest.version}.\n{% /card-link %}`,
