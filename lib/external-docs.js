...
const SOURCE_FIELDS = new Set(['id', 'title', 'repo', 'ref', 'path', 'trackRelease']);
...
function normalizeSource(rawSource, index, allowedOrgs) {
  ...
  if (unsupportedFields.length > 0) {
    throw new Error(`Invalid docs-sources.yml: ${label} contains unsupported field${unsupportedFields.length === 1 ? '' : 's'} ${unsupportedFields.map((field) => `"${field}"`).join(', ')}. Use only id, title, repo, ref, path, and trackRelease.`);
  }
  ...
  if (rawSource.trackRelease !== undefined && typeof rawSource.trackRelease !== 'boolean') {
    throw new Error(`Invalid docs-sources.yml: source "${id}" trackRelease must be a boolean.`);
  }
  ...
  return {
    ...
    ...(rawSource.trackRelease !== undefined ? { trackRelease: rawSource.trackRelease } : {}),
  };
}
