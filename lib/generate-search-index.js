...
/**
 * Read a frontmatter field as a string while treating absent and falsy values as empty metadata.
 */
...
function getSection(path) {
  ...
  if (path === "/changes" || path.startsWith("/changes/") || path === "/reference/releases") {
    return "Changes";
  }
  if (
  ...
  if (path.startsWith("/tools")) return "Reference";
  if (path.startsWith("/architecture")) return "Reference";
...
/**
 * Build the search index from resolved local and external page Markdown.
 */
async function main() {
  ...
  for (const file of files) {
    ...
    const { markdown: content } = resolvePageDocument(
      frontmatter,
      localContent,
      externalContent,
