...
function extractFrontmatter(content) {
  ...
  if (!fmMatch) return { title: "", description: "", rawUrl: "" };

  const frontmatter = fmMatch[1];
  const titleMatch = frontmatter.match(/^title:\s*(.+)$/m);
  const descMatch = frontmatter.match(/^description:\s*(.+)$/m);
  const rawUrlMatch = frontmatter.match(/^rawUrl:\s*(.+)$/m);

  return {
    title: titleMatch ? stripYamlValue(titleMatch[1]) : "",
    description: descMatch ? stripYamlValue(descMatch[1]) : "",
    rawUrl: rawUrlMatch ? stripYamlValue(rawUrlMatch[1]) : "",
  };
}
...
async function getIndexContent(file, localContent, rawUrl) {
  if (!rawUrl) return localContent;

  const response = await fetch(rawUrl, { cache: "no-store" });
  if (!response.ok) {
    throw new Error(`Failed to fetch rawUrl for search index ${relative(PAGES_DIR, file)}: ${rawUrl} returned ${response.status} ${response.statusText || ""}`.trim());
  }
...
async function main() {
  ...
  for (const file of files) {
    ...
    const { title, description, rawUrl } = extractFrontmatter(localContent);
    const content = await getIndexContent(file, localContent, rawUrl);
